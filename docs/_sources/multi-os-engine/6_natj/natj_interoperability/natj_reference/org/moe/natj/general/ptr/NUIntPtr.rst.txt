...
.. java:type:: public interface NUIntPtr extends Ptr<Long>, ConstNUIntPtr

   Architecture dependent unsigned Long pointer interface. - On a 32-bit platform the element size is 4 bytes - On a 64-bit platform the element size is 8 bytes
...
.. java:method:: @Override public NUIntPtr getGuarded(int length)
   :outertype: NUIntPtr

getGuarded
^^^^^^^^^^

.. java:method:: @Override public NUIntPtr getGuarded(int fromIndex, int toIndex)
   :outertype: NUIntPtr

ofs
^^^

.. java:method:: @Override public NUIntPtr ofs(int elemOffset)
   :outertype: NUIntPtr
